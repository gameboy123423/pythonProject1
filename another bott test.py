...
def sales_per_day():
    ...
    grouped = sales_data.groupby(['date', 'goods_name']).agg({'amount': 'sum', 'price': 'sum'}).reset_index()
    return grouped
...
def sales_per_week():
    ...
    grouped = sales_data.groupby(['year', 'week', 'goods_name']).agg({'amount': 'sum', 'price': 'sum'}).reset_index()
    return grouped
...
def sales_per_month():
    ...
    grouped = sales_data.groupby(['month', 'goods_name']).agg({'amount': 'sum', 'price': 'sum'}).reset_index()
    return grouped

# Функция отправки сводки продаж
def send_sales_summary(message, period):
    print(f"Sending sales summary for {period}")  # Log period
    if period == 'day':
    ...
    for index, row in sales_data.iterrows():
        ...
        if period == 'day':
            output += f"{row['date']} — {row['goods_name']}: {amount} шт, {sum_price} тг\n"
        elif period == 'week':
            output += f"{row['year']}-W{row['week']} — {row['goods_name']}: {amount} шт, {sum_price} тг\n"
        elif period == 'month':
            output += f"{row['month']} — {row['goods_name']}: {amount} шт, {sum_price} тг\n"

    output += f"\nОбщее количество денег за {period}: {total_sum}\n"
